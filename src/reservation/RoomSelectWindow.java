...
public class RoomSelectWindow extends Frame implements Runnable, Observable {
	 ...
	 */
	public RoomSelectWindow(int roomtype, int startday, int endday, ArrayList reservations) {
		// this.rm = rm;
	...
	/*
	 * private void loadFile() { try { FileInputStream fis = new
	 * FileInputStream("./db/restable.jh"); ObjectInputStream ois = new
	 * ObjectInputStream(fis);
	 * 
	 * this.reservations = (ArrayList) ois.readObject(); ois.close(); } catch
	 * (ClassNotFoundException cnf) { System.out.println(cnf +
	 * " Reservation.makeReservation()"); } catch (IOException io) {
